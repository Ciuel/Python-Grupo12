...
def correct_button_match(window: sg.Window, info_partida: dict,
                         not_found_elements: list, value_matrix: np.array,
    ...
    Args:
        ...
        not_found_elements (list): La lista de las fichas de la partida seleccionables por la ayuda
    ...
    Returns:
        [tuple]: not_found_elements y info_partida
    """
    ...
    not_found_elements.remove(primer_elemento)

    info_partida["lista_chequeos"] = []

    return not_found_elements, info_partida


def incorrect_button_match(window:sg.Window, info_partida:dict, type_token:str):
    ...
    for eve in info_partida["lista_chequeos"]:
        window[eve].update(
            ""
        ) if type_token == "Text" else window[eve].update(
...
def check_button(value_matrix: np.array, user: dict, info_partida: dict,
                 ...
                 not_found_elements: list, nick: str, game_number: int,
                 vlc_dict: dict):
    ...
    if all(primer_elemento == value_matrix[int(x[-2])][int(x[-1])]
        ...
        if len(info_partida["lista_chequeos"]
            ...
            not_found_elements, info_partida = correct_button_match(
                window, info_partida, not_found_elements, value_matrix,
                primer_elemento, user["config"]["Coincidences"])
    ...
    return info_partida, not_found_elements


def button_amount(user_config: dict) -> int:
    return (LEVEL_DICTIONARY[(user_config["Level"],user_config["Coincidences"])][0] *LEVEL_DICTIONARY[(user_config["Level"],user_config["Coincidences"])][1])


def win_game(window, info_partida, nick, user, tiempo_total, game_number,
             vlc_dict):
    ...
    if info_partida["hits"] >= button_amount(user["config"]) // user["config"]["Coincidences"]:
        ...
        info_partida["points"] += 30 * (tiempo_dado - tiempo_total)
        window.close()
        send_info(time.time(), game_number, "fin", user, nick,info_partida["points"], "finalizada")
        score.start(user["config"]["AppColor"], nick,user["config"]["VictoryText"], tiempo_total,info_partida["hits"], info_partida["misses"],
                    info_partida["points"], vlc_dict)


def lose_game(window, info_partida, nick, user, tiempo_total, game_number,vlc_dict):
    tiempo_dado = 30 * user["config"]["Coincidences"] * user["config"]["Level"]
    if tiempo_total == tiempo_dado:
        vlc_play_sound(vlc_dict, LOSE_SOUND_PATH)
        window.close()
        send_info(time.time(), game_number, "fin", user, nick,info_partida["points"], "timeout")
        score.start(user["config"]["AppColor"], nick,
                    user["config"]["LoseText"], tiempo_total,
...
def help_action(window, value_matrix, type_of_token, not_found_elements):
    window["-HELP-"].update(disabled=True)
    window.refresh()
    obj = random.choice(not_found_elements)
    help_list = []
...
def check_help(window, event, value_matrix, type_of_token, not_found_elements,vlc_dict):
    if event == "-HELP-":
        vlc_play_sound(vlc_dict, HELP_SOUND_PATH)
        help_action(window, value_matrix, type_of_token, not_found_elements)
        return True
    else:
        return False


def send_info(timestamp: float,game_number: int,event: str,user: dict,nick: str,points: int,state: str = "",token: str = "") -> None:
