...
def round_btn(button_text,
              ...
              button_width=None):
    multi = 4
    btn_w = ((len(button_text) if button_width is None else button_width) * 5 +
             20) * multi
    height = 20 * multi
    ...
    return Sg.Button(button_text=button_text,
                     image_data=btn_img,
                     button_color=(text_color, text_color),
                     tooltip=tooltip,
                     key=key,
                     pad=pad,
                     enable_events=False,
...
def find_in_file(string,
    ...
    for file in demo_files_dict:
        try:
            ...
            with open(full_filename, 'rb',
                      0) as f, mmap.mmap(f.fileno(),
                                         0,
        ...
        for key in matched_dict:
            ...
            try:
                for _match in matched_dict[key]:
                    ...
                    if (verbose):
                        sg.cprint(f"Line: {line_num_match} ",
                                  c='white on purple',
                                  end='')
...
def window_choose_line_to_edit(filename, full_filename, line_num_list):
    ...
    for line in sorted(set(line_num_list)):
        layout += [[
            sg.Text(f'Line {line}', key=('-T-', line), enable_events=True)
        ]]
    layout += [[sg.B('Cancel')]]
    ...
    while True:
        ...
        if event == 'Edit' or event.endswith('+Edit+'):
            ...
            for file in values['-DEMO LIST-']:
                if find_in_file.file_list_dict is not None:
                    full_filename, line = window_choose_line_to_edit(
                        file, find_in_file.file_list_dict[file][0],
                        find_in_file.file_list_dict[file][1])
                else:
                    full_filename, line = get_file_list_dict()[file], 1
                if line is not None:
                    sg.cprint(f'Editing using {editor_program}',
                              text_color='white',
                    ...
                    if using_local_editor():
                        execute_command_subprocess(editor_program,
                                                   full_filename)
                    else:
                        try:
            ...
            for file in values['-DEMO LIST-']:
                ...
                sg.cprint(file_to_run,
                          text_color='white',
                          background_color='purple')
                try:
                    sp = execute_py_file(f'{file_to_run}',
                                         pipe_output=values['-WAIT-'])
                except TypeError:
                    sg.cprint(
                        'Consider upgrading to a newer PySimpleGUI.... 4.37.0 has better execution controls',
                        c='white on red')
                    sp = execute_py_file_with_pipe_output(
                        f'{file_to_run}', pipe_output=values['-WAIT-'])
                try:
                    if values['-WAIT-']:
                        sg.cprint(f'Waiting on results..',
                                  text_color='white',
                                  background_color='red',
                                  end='')
                        while True:
            ...
            new_list = [
                ...
                if values['-FILTER-'].lower() in i.lower()
            ]
            ...
            if values['-FIND-']:
                ...
                elif current_typed_value.startswith(
                        old_typed_value) and old_ignore_case is is_ignore_case:
                    old_typed_value = current_typed_value
                    file_list = find_in_file(
                        values['-FIND-'],
                ...
                window['-ML-'].update('')
                file_list = find_in_file(values['-FIND RE-'],
                                         get_file_list_dict(),
                                         regex=True,
                                         verbose=values['-VERBOSE-'],
                                         window=window)
            ...
            if explorer_program:
                sg.cprint(f'Opening Folder using {explorer_program}...',
                          c='white on green',
                          end='')
                sg.cprint('')
                for file in values['-DEMO LIST-']:
...
def execute_py_file_with_pipe_output(pyfile,
                                     parms=None,
                                     cwd=None,
    ...
    try:
        if interpreter_command is not None:
            ...
            python_program = sg.pysimplegui_user_settings.get(
                '-python command-', '')
    except:
        python_program = ''

    if python_program == '':
        python_program = 'python' if sys.platform.startswith(
            'win') else 'python3'
    if parms is not None and python_program:
        sp = execute_command_subprocess_with_pipe_output(
            python_program,
            pyfile,
            parms,
            wait=wait,
...
def execute_command_subprocess_with_pipe_output(command,
                                                *args,
                                                wait=False,
                                                cwd=None,
                                                pipe_output=False):
    ...
    try:
        if args is not None:
            ...
            if pipe_output:
                sp = subprocess.Popen(command + ' ' + expanded_args,
                                      shell=True,
...
if __name__ == '__main__':
    try:
        ...
        major_version, minor_version = int(version_parts[0]), int(
            version_parts[1])
